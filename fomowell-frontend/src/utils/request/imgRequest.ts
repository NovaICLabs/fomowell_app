...
class RequestInstance {
  ...
  post<T>(params: requestParamsData): Promise<T> {
    return this.instance.post(params.url, params.data, params.requestConfig).then((response) => {
      if ('data' in response) {
...
const Request = new RequestInstance({
  baseURL: process.env.NODE_ENV === 'production' ? 'https://image.fomowell.com/api' : '/api',
  timeout: 300000,
  interceptors: {
    ...
    responseInterceptor: <T>(response: AxiosResponse<any>): T => {
